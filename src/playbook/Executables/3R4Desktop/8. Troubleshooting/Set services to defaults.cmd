...
set "servicesPath=%windir%\3R4Desktop\6. Advanced Configuration\Services"
if not exist "%servicesPath%" (
	echo Services in 3R4 folder not found, can't continue.
	if "%*"=="" pause
	exit /b 1
)
...
echo This will reset the configuration of services in the 3R4 folder.
...
:main
%title% "Enabling services in the 3R4 folder... This might take a while."
for /f "usebackq tokens=*" %%a in (`dir /b /s "%windir%\3R4Desktop\6. Advanced Configuration\Services" ^| find "(default)"`) do (
	call :run "%%a"
	start /min /high /wait cmd /c "%%a" /silent
)

set "3R4Other=%windir%\3R4Modules\Other"
set "winServices=%3R4Other%\winServices.reg"
set "3R4Services=%3R4Other%\3R4Services.reg"
if exist "%winServices%" (
	if exist "%3R4Services%" call :fullRestore
)
...
echo 2) Restore a full services backup of the default 3R4 services
echo 3) Nothing
echo]
choice /c:123 /n /m "Choose a number: [1/2/3] "
if "%errorlevel%"=="1" reg import "%winServices%" > nul
if "%errorlevel%"=="2" reg import "%3R4Services%" > nul
exit /b
