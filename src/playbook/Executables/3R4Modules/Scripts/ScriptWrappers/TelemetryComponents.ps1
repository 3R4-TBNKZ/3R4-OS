...
$packageInstall = "$windir\3R4Modules\Scripts\packageInstall.ps1"
...
$package = "*Z-3R4-NoTelemetry-Package*"
...
function Menu {
	...
	Write-Host "This script adds or removes 3R4' NoTelemetry package, which removes certain telemetry components in Windows.`n" -ForegroundColor Cyan

	Write-BulletPoint @"
Removing the package restores the telemetry components, which can aid in troubleshooting. If
   resolving the issue involves removing the package, please report this as an issue to 3R4.`n
"@
	Write-BulletPoint @"
3R4 retains policies that should disable telemetry even after the package removal. These policies
