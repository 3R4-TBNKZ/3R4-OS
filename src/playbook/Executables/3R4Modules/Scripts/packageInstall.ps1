...
param (
	...
	[string]$PackagesPath = "$([Environment]::GetFolderPath('Windows'))\3R4Modules\Packages",
...
& "$windir\3R4Modules\initPowerShell.ps1"
$sys32 = [Environment]::GetFolderPath('System')
$safeModePackageList = "$sys32\safeModePackagesToInstall.3R4module"
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($NoInteraction) {
			...
			$failedMsgTitle = '3R4FailedComponentMsgBox'
			$failedMsgArgs = "/c title Finalizing Installation - 3R4 & echo Do not close this window. & schtasks /delete /tn `"$failedMsgTitle`" /f > nul & " `
		...
		if ($safeModeStatus) {
			Write-Host "Please report this to the 3R4 team, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to 3R4." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to 3R4." -ForegroundColor Red
...
Would you like 3R4 to restart your system into Safe Mode and try again? This process shouldn't take much time.

Please note that if you chose to disable Windows Defender, it may still remain enabled if you select 'No'. However, you can always try disabling it later in the 3R4 folder.
"@

	if ((Read-MessageBox -Title "3R4 - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
